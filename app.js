...
io.on("connection", function(socket) {
    ...
    socket.on("request-help", function(userLocation) {
        ...
        if (closestDriverId) {
            clients[closestDriverId].isAvailable = false;
    ...
    socket.on("driver-available", function() {
        if (clients[socket.id]) clients[socket.id].isAvailable = true;
    });
    ...
    socket.on("disconnect", function() {
        if (clients[socket.id]) {
            if (clients[socket.id].role === 'driver') io.emit("driver-disconnected", socket.id);
